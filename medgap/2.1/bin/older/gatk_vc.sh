...
OUTPUT_MODE_UG=""
OUTPUT_MODE_HC=""
if [[ "$reference_calls" == "GVCF" ]]
then
        OUTPUT_MODE_HC="-ERC GVCF -variant_index_type LINEAR -variant_index_parameter 128000"
        NO_VARIATION="-selecttype no_variation"
fi

if [[ "$reference_calls" == "BIT" ]]
then
        OUTPUT_MODE_HC="-ERC BP_RESOLUTION"
        NO_VARIATION="-selecttype no_variation"
fi
if [[ "$reference_calls" == "ALL" ]]
then
        OUTPUT_MODE_UG="--output_mode EMIT_ALL_SITES"
        NO_VARIATION="-selecttype no_variation"
fi
if [[ "$reference_calls" == "CONF" ]]
then
        OUTPUT_MODE_UG="--output_mode EMIT_ALL_CONFIDENT_SITES"
        NO_VARIATION="-selecttype no_variation"
fi
